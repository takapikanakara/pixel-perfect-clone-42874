// Shared helper to forward gateway webhook events to UTMify.
// UTMify Orders API: POST https://api.utmify.com.br/api-credentials/orders
...
function toCents(value: any): number {
  ...
  // If looks like already-in-cents (integer >= 1000 with no decimals), keep it.
  // Otherwise treat as currency unit (EUR/BRL) and convert.
...
function mapPaymentMethod(value: any): "pix" | "credit_card" | "billet" | "free_price" {
  ...
  // mbway and other gateways → fall back to pix-like instant payment slot
  return "pix";
}
...
export async function forwardToUtmify(
  ...
  const tx = pick<AnyRecord>(rawBody, "transaction", "data", "deposit", "payment") ?? rawBody;
